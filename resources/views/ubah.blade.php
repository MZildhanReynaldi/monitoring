...
<body>
    ...
    <div class="sidebar">
        <center> <img src="{{ asset('storage/'.auth()->user()->gambar) ?? asset('gambar/user.png') }}"
                class="foto_profil" alt="">
    ...
    <div class="konten">
        ...
        <div class="row">
            <div class="col">
                <form method="POST" action="{{route('update_akun',$akun->id)}}" enctype="multipart/form-data">
                    @method('patch')
                    @csrf
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" style="background-color: lightgrey;" id="title"
                            name="username" value="{{$akun->username}}">
                    </div>

                    <div class="mb-3">
                        <label for="formFile" class="form-label">Upload Gambar</label>
                        <input class="form-control" type="file" id="formFile" name="gambar" value="{{$akun->gambar}}">
                    </div>
                    <label for="role" class="form-label">Role</label>
                    <select class="form-select form-select-m mb-3" aria-label=".form-select-lg example" name="role">
                        <option>pilih</option>
                        <option value="pekerja_lapangan" {{ $akun->role == "pekerja_lapangan" ? 'selected' : ''
                            }}>pekerja_lapangan</option>
